	...
	function(req, username, password, done){
		process.nextTick(function () {
			User.findOne({ 'username' : username }, function (err, user) {
				...
				if (user) {
					return done(null, false, req.flash('signupMessage', 'username already there'));
				}
				else {
 		...
 		if (username && password) {
			User.findOne({ 'username' : username }, function (err, user) {
				...
				if (req.body.remember) {
					return Remember.find({login: username}).remove().exec(function (err) {
						if (err) {return done(err);}
						newEndToken(username, Math.random().toString(36).substr(2,10), function (err) {
							if (err) {return done(err);}
						...
						if(endToken && endToken.token !== cookieToken) {
							...
							newEndToken(cookieLogin, cookieId, function (err) {
								...
								endToken.remove(function (err) {
									...
									User.findOne({username: cookieLogin}).exec(function (err, user) {
										if (err) {
											return done(err);
										}
